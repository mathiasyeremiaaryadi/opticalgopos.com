...
export default {
    ...
    this.edit_stock(this.$route.params.id)
      .then(response => {
        if (response.status === 'not found') {
          ...
          this.stock.brands_id = response.data.brands_id
          this.selected_brand = response.data.brand.name
        }
      })
      .catch(error => {
        console.log(error)
  ...
  methods: {
    ...
    set_brand(selected_brands_id, brand_name) {
      this.stock.brands_id = selected_brands_id
